/*#include "receiver.h"

ReceiverClass Receiver;

bool ReceiverClass::ledIsOn() {
    return analogRead(LDR_PIN) < threshold;
}

void ReceiverClass::init() {
    pinMode(BUTTON_PIN, INPUT_PULLUP);
    Serial.begin(9600);
}

void ReceiverClass::calibrateThreshold() {
    Serial.println("Threshold setzen: Taste drücken...");
    while (digitalRead(BUTTON_PIN) == HIGH);
    threshold = analogRead(LDR_PIN) - THRESHOLD_OFFSET;
    Serial.print("Threshold: ");
    Serial.println(threshold);
}

bool ReceiverClass::detectStartSignal() {
    if (ledIsOn()) return false;

    unsigned long t = millis();
    while (!ledIsOn()) {
        if (millis() - t > 100) {
            delay(100);
            Serial.println("Start detectet");
            return true;
        }
    }
    return false;
}


void ReceiverClass::sync() {
    nextSample = micros() + (STEP_TIME * 1000) / 2;
    Serial.println("we are snyced");
}



String ReceiverClass::read() {
    receivedText = "";
    endCheckStart = 0;

    while (true) {

        // --- 1. STARTBIT LESEN ---
        while (micros() < nextSample);
        nextSample += STEP_TIME * 1000;

        // Startbit MUSS 0 sein
        if (ledIsOn()) {
            continue; // falsches Startbit -> Zeichen überspringen
        }



        // --- 2. 7 DATENBITS LESEN ---
        byte current = 0;

        for (int i = 0; i < 7; i++) {
            while (micros() < nextSample);
            nextSample += STEP_TIME * 1000;

            current = (current << 1) | (ledIsOn() ? 1 : 0);
        }

        // --- 3. STOPBIT IGNORIEREN ---
        while (micros() < nextSample);
        nextSample += STEP_TIME * 1000;

        // --- 4. ZEICHEN SPEICHERN ---
        receivedText += (char)current;

        if (ledIsOn()) {
            if (endCheckStart == 0) endCheckStart = millis();
            if (millis() - endCheckStart > END_TIME){
                Serial.print("Read finished ");
                Serial.println("End detectet");
                return receivedText;
            } 
        }
        else {
            endCheckStart = 0;
        }
        Serial.println("1 readcycle done");

    }
}



void ReceiverClass::showText(String txt) {
    Serial.println(txt);
}*/
/*
#include "receiver.h"

ReceiverClass Receiver;

bool ReceiverClass::ledIsOn() {
    return analogRead(LDR_PIN) > threshold;
}

void ReceiverClass::init() {
    pinMode(BUTTON_PIN, INPUT_PULLUP);
    Serial.begin(9600);
}

void ReceiverClass::calibrateThreshold() {
    Serial.println("Threshold setzen: Taste drücken...");
    while (digitalRead(BUTTON_PIN) == HIGH);
    threshold = analogRead(LDR_PIN) - THRESHOLD_OFFSET;
    Serial.print("Threshold: ");
    Serial.println(threshold);
    delay(500);
}

bool ReceiverClass::detectStartSignal() {
    if (ledIsOn()) return false;

    unsigned long t = millis();
    while (!ledIsOn()) {
        if (millis() - t > 998) {
            Serial.println("Start detectet");
            return true; // Startsignal erkannt
        }
    }
    return false;
}

void ReceiverClass::sync() {
    nextSample = millis() + (STEP_TIME / 2);
    Serial.println("We are synced");
}

String ReceiverClass::read() {
    receivedText = "";
    endCheckStart = 0;

    byte current = 0; //hier werden bits eines Zeichens gesammelt
    int count = 0; //wie viele bits in current drinnen sind (wenn =7, dann ein zeichen)

    while (true) {
        while (millis() < nextSample);
        nextSample += STEP_TIME; //festlegen bis wann der durchlauf geht

        if (ledIsOn()) continue; // Startbit muss 0 sein


        bool bit = ledIsOn();//aktuelllen bit lesen(true/false)

        current = (current << 1) | (bit ? 1 : 0);// bisherigen Wert um 1 Bit nach links schieben und das neue Bit anhängen
        count++;//hochsetzen um +1


        if (count == 7) { //komplettes Zeichen erkennen
            receivedText += (char)current; //in Zeichen umwandeln
            current = 0;//variablen zurücksetzen für nächstes zeichen
            count = 0;//variablen zurücksetzen für nächstes zeichen
        }

        //rausgehen wenn led END_TIME lan an ist
        if (ledIsOn()) {
            if (endCheckStart == 0) endCheckStart = millis();
            if (millis() - endCheckStart > END_TIME) {
                Serial.println("End detectet");
                return receivedText; 
            }
        } else {
            endCheckStart = 0;
        }
        Serial.println("1 more Readcycle done");
    }
}


void ReceiverClass::showText(String txt) {
    Serial.println(txt);
}*/


#include "receiver.h"
#include "parameter.h"

void Receiver::begin() {
    pinMode(LDR_PIN, INPUT);
}

void Receiver::calibrate() {
    int val = analogRead(LDR_PIN);
    threshold = val + THRESH_MARGIN;
}

bool Receiver::detectStart() {
    int v = analogRead(LDR_PIN);
    return v > threshold; // LED an = Start
}

bool Receiver::detectStop() {
    int v = analogRead(LDR_PIN);
    return v < threshold; // LED aus = Stop
}

int Receiver::readChar() {
    while (!detectStart()) {}

    delay(START_TIME);

    uint8_t value = 0;

    for (int i = 0; i < 6; i++) {
        delay(BIT_TIME);
        int v = analogRead(LDR_PIN);
        bool bit = v > threshold;
        value |= (bit << i);
    }

    while (!detectStop()) {}

    delay(STOP_TIME);

    uint8_t decrypted = value ^ XOR_KEY;
    return decrypted;
}
